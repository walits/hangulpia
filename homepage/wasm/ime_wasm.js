 ...
 * @returns {string}
 */
export function convert_hiragana_only(input) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(input, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.convert_hiragana_only(ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
 ...
 * @returns {number}
 */
export function vocab_size() {
    const ret = wasm.vocab_size();
    return ret >>> 0;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('ime_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
