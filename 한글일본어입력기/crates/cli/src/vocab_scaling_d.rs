//! ARM D only: Large vocab + optimal config
//! Split out because the full benchmark times out.
...
fn main() {
    ...
    println!("━━━ [D] 확장어휘 + 최적조합 (단독 실행) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    let global_start = Instant::now();

    let vocab_large = build_vocab_large();
    println!("  어휘: {} 개, 테스트: {} 건, 학습량: {}K", vocab_large.len(), test_count, train_vol / 1000);

    let cfg = TrainerConfig { dim: 64, iterations: 50, ..Default::default() };
    let test = generate_corpus_with_seed(&vocab_large, test_count, 99);
    ...
    eprint!("  음소맵...");
    let t = Instant::now();
    let pmap = build_phonetic_map_chunked(&vocab_large, train_vol);
    eprintln!(" {:.1}s", t.elapsed().as_secs_f64());

    eprint!("  N-gram...");
    let t = Instant::now();
    let ngram = build_ngram_model_chunked(&vocab_large, train_vol);
    eprintln!(" {:.1}s — {}", t.elapsed().as_secs_f64(), ngram.stats());

    eprint!("  평가...");
    let t = Instant::now();
    let m = bench(&test, &db, &ngram, &pmap, 0.1, 0.35, 0.25, 0.3);
    eprintln!(" {:.1}% ({:.1}s)", m.exact_pct(), t.elapsed().as_secs_f64());

    println!();
    println!("  ┌─────────────────┬────────┬────────┬────────┬────────┬────────┬────────┐");
    println!("  │ 메트릭           │ Hira   │ Hira   │ Top-1  │ Top-3  │ Top-10 │  Doc   │");
    println!("  │                  │ Hit    │ Top-1  │ Surf   │ Hit    │ Hit    │ Match  │");
    println!("  ├─────────────────┼────────┼────────┼────────┼────────┼────────┼────────┤");
    println!("  │ 확장어휘+최적    │ {:>5.1}% │ {:>5.1}% │ {:>5.1}% │ {:>5.1}% │ {:>5.1}% │ {:>5.1}% │",
        m.hira_hit_pct(), m.hira_top1_pct(), m.exact_pct(), m.top3_pct(), m.top10_pct(), m.doc_pct());
    println!("  └─────────────────┴────────┴────────┴────────┴────────┴────────┴────────┘");
    println!();
    println!("  총 단어수: {}, 소요시간: {:.1}s", m.total_words, global_start.elapsed().as_secs_f64());
}
