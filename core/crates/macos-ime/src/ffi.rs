...
pub extern "C" fn hj_engine_init(db_path: *const c_char) -> i32 {
    ...
    // Build vocabulary and populate dictionary. Combines vocab.rs +
    // vocab_extended.rs + vocab_gapfill.rs + vocab_jlpt.rs (4,153 entries,
    // all verified real words — vocab_jlpt.rs adds 2,432 JLPT N5/N4/N3
    // words sourced from a public MIT-licensed word list and cross-checked
    // against the other three files for duplicates, see docs/MODEL.md
    // section 1.4). vocab_large.rs is deliberately NOT included: an audit
    // found ~81% of its unique "extended" entries (18,400 of 22,741) are
    // mechanically generated by pairing ~340 stem kanji with ~20 suffix
    // kanji in a full cartesian product (e.g. 食 alone pairs with 344
    // distinct stems — no real dictionary does that), producing words like
    // 読学/置学 that don't exist. That noise showed up as bogus kanji
    // candidates in hj_convert(), and removing it also trims the
    // PhoneticMap's average per-token ambiguity from 1.5 to 1.2 (fewer
    // conflicting candidates). See docs/MODEL.md section 6.3/1.4.
