...
use ime_db::vocab_jlpt::{build_jlpt_n3_vocab, build_jlpt_n4_vocab, build_jlpt_n5_vocab};
...
/// Combines vocab.rs + vocab_extended.rs + vocab_gapfill.rs + vocab_jlpt.rs
/// (4,153 entries, all verified real words — vocab_jlpt.rs adds 2,432 JLPT
/// N5/N4/N3 words sourced from a public MIT-licensed word list and
/// cross-checked against the other three files for duplicates, see
/// docs/MODEL.md section 1.4). vocab_large.rs is deliberately NOT included:
/// an audit found ~81% of its unique "extended" entries (18,400 of 22,741)
/// are mechanically generated by pairing ~340 stem kanji with ~20 suffix
/// kanji in a full cartesian product (e.g. 食 alone pairs with 344 distinct
/// stems — no real dictionary does that), producing words like 読学/置学
/// that don't exist. See docs/MODEL.md section 6.3/1.4.
///
/// Returns the kanji-dict entries (tiered by source, not a single flat
/// number: hand-curated "core" words rank above JLPT vocabulary, which is
/// in turn ranked by JLPT level itself — N5 is by design the most common
/// tier, N3 the least common of the three. This is a real, defensible
/// frequency proxy, not a fabricated corpus count: it's exactly why the
/// JLPT levels are ordered the way they are. It replaces the flat
/// frequency=100 every entry used to get, which made KanjiDict's
/// "ORDER BY frequency DESC" a no-op for homophones — e.g. こうえん's
/// 公園/公演/講演 were all tied and came back in arbitrary insertion order.
/// See docs/MODEL.md section 1.4.2 / 6.1, paper section 2.3) alongside the
/// flat combined vocab list (used for PhoneticMap training, where weight
/// is intentionally left flat — see hj_engine_init).
fn tiered_vocab_and_kanji_entries() -> (Vec<(&'static str, &'static str, i64)>, Vec<ime_db::vocab::VocabEntry>) {
    const FREQ_CORE: i64 = 1000;
    const FREQ_N5: i64 = 500;
    const FREQ_N4: i64 = 300;
    const FREQ_N3: i64 = 100;

    let mut core_vocab = build_vocab();
    core_vocab.extend(build_extended_vocab());
    core_vocab.extend(build_gapfill_vocab());
    let n5_vocab = build_jlpt_n5_vocab();
    let n4_vocab = build_jlpt_n4_vocab();
    let n3_vocab = build_jlpt_n3_vocab();

    let mut kanji_entries: Vec<(&'static str, &'static str, i64)> = Vec::new();
    kanji_entries.extend(core_vocab.iter().map(|v| (v.reading, v.surface, FREQ_CORE)));
    kanji_entries.extend(n5_vocab.iter().map(|v| (v.reading, v.surface, FREQ_N5)));
    kanji_entries.extend(n4_vocab.iter().map(|v| (v.reading, v.surface, FREQ_N4)));
    kanji_entries.extend(n3_vocab.iter().map(|v| (v.reading, v.surface, FREQ_N3)));

    let mut vocab = core_vocab;
    vocab.extend(n5_vocab);
    vocab.extend(n4_vocab);
    vocab.extend(n3_vocab);
...
pub extern "C" fn hj_engine_init(db_path: *const c_char) -> i32 {
    ...
    // Build vocabulary and populate dictionary — see
    // tiered_vocab_and_kanji_entries() doc comment for what's combined and why.
    let (kanji_entries, vocab) = tiered_vocab_and_kanji_entries();
    let dict = db.kanji_dict();
    let _ = dict.insert_batch(&kanji_entries);

    // Build phonetic map from vocabulary. Alignment-training weight is kept
    // flat here on purpose (unlike the kanji frequency above): weighting it
    // by tier would shift which candidate wins ambiguous phoneme alignments
    // across the whole trained model, which needs its own dedicated testing
    // pass rather than riding along with the kanji-candidate-ranking fix.
...
mod tests {
    ...
    /// こうえん has three real homophones across our vocab (公園 in the
    /// hand-curated core, 講演/公演 in JLPT N3) — confirms tiered frequency
    /// actually breaks the tie in a sensible direction (park, the more
    /// common everyday word, outranks the two N3-level words) instead of
    /// the old flat frequency=100 that left ordering to chance.
    #[test]
    fn test_kouen_homophones_ranked_by_tier() {
        let (kanji_entries, _vocab) = tiered_vocab_and_kanji_entries();
        let db = DictionaryDb::open(":memory:").unwrap();
        let dict = db.kanji_dict();
        dict.insert_batch(&kanji_entries).unwrap();

        let results = dict.lookup("こうえん").unwrap();
        assert!(results.len() >= 3, "expected at least 3 homophones for こうえん");
        assert_eq!(results[0].surface, "公園");
